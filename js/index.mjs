...
import {
  getAllPostsApiEndpoint,
  getAllPostsByTagApiEndpoint,
    ...
    try {
      ...
      if (allPosts.length > 0) {
        ...
        tagDropdown.addEventListener("change", async () => {
          ...
          if (selectedTag) {
            const filteredPostsByTag = await fetchPostsByTag(
              username || defaultPublicUsername,
              selectedTag
            );
...
async function fetchPostsByTag(username, tag) {
  try {
    ...
    const response = await fetch(
      getAllPostsByTagApiEndpoint(username || defaultPublicUsername, tag),
      {
        method: "GET",
        headers,
      }
...
function displayPosts(posts, page, postsPerPage) {
  ...
  currentPosts.forEach((post) => {
    const imageUrl =
      post.media && post.media.url ? post.media.url : "/assets/placeholder.jpg";
